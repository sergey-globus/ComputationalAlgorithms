...
def getPolynomSurface(x, y, koefs: list):
    return koefs[0] + koefs[1] * x + koefs[2] * y if np.shape(koefs)[0] == 3 else koefs[0] + koefs[1] * x + koefs[2] * y + koefs[3] * x * y + koefs[4] * x ** 2 + koefs[5] * y ** 2 
...
def getPolynom(x, n: int, koefs: list):
    return 1 - x + koefs[0] * x * (1 - x) + koefs[1] * x ** 2 * (1 - x) if n == 2 else 1 - x + koefs[0] * x * (1 - x) + koefs[1] * x ** 2 * (1 - x) + koefs[2] * x ** 3 * (1 - x)
...
def solveODE():
    ...
    # m = 2
    alphas = -2 + 2 * x - 3 * x ** 5
    bethas = 2 - 5 * x + 2 * x ** 2 - 4 * x ** 3
    alphas_bethas = alphas * bethas
    
    koefsA = np.array([[np.sum(alphas ** 3), np.sum(alphas_bethas)], [np.sum(alphas_bethas), np.sum(bethas ** 2)]])
   
    sum_1 = alphas * (4 * x - 1)
    sum_2 = bethas * (4 * x - 1)
    
    koefsB = np.array([[np.sum(sum_1)], [np.sum(sum_2)]])
